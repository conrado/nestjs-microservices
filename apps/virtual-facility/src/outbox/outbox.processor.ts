...
export class OutboxProcessor {
  ...
  async processOutboxMessages() {
    ...
    const outboxMessages = await this.outboxService.getUnprocessedMessages({
      target: WORKFLOWS_SERVICE.description,
      take: 100,
    });
    await Promise.all(
      outboxMessages.map(async (message) => {
        await this.dispatchWorkflowEvent(message);
        // instead of deleting the message, we could
        // implement it being marked as processed
